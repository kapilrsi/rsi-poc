<script lang="ts">
    ...
    import { ehrbase, ehrscape } from "../fhir";
    ...
    const handleSubmit = async (e: CustomEvent) => {
        ...
        try {
            ...
            console.log(config);
            if (config) {
                const { username, password, ehrId, patientId, appointmentId } = config;
                // let baseURL = ehrscape;
    ...
    const createEHR = async (e: CustomEvent) => {
        const createEHR = e.detail;
        console.log({ createEHR });
        ...
        try {
            ...
            if (config) {
                ...
                const response = await opener.post(
                    "/ehr",
                    "",
                    {
                        params: { 'Prefer': "return=representation", 'Content-Type': 'application/json'},
                    }
                );
